  ...
  try {
    ...
    browser.on('disconnected', () => {
      if (!state.closing) {
        ...
        console.error(`[WebKit attempt ${attempt}] browser disconnected unexpectedly.`);
      }
    });
    ...
    page.on('crash', () => {
      ...
      console.error(`[WebKit attempt ${attempt}] page crashed unexpectedly.`);
    });
    page.on('pageerror', (error) => {
      console.error(`[WebKit attempt ${attempt}] page error: ${error?.message || error}`);
    });
    ...
    if (isRetryableWebKitCrash(error, state)) {
      error.webkitInfrastructureCrash = true;
    }
    throw error;
  } finally {
    state.closing = true;
...
for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
  try {
    ...
    const canRetry = Boolean(error?.webkitInfrastructureCrash) && attempt < maxAttempts;
    if (!canRetry) throw error;

    recoveredFromCrash = true;
    console.error(`[WebKit attempt ${attempt}] infrastructure-level browser/page crash detected; retrying once with a fresh browser.`);
  }
}
