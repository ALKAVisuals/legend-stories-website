...
test('delegated image errors trigger recovery and clean up listeners', async () => {
  const page = 'sport-lions-pride.html';
  const replacement = '/assets/lions-pride-current.webp';
  ...
  const controller = initCartControlDelegation({
    ...
    onUpdateQuantity: () => {},
    onRemoveItem: () => {},
    baseUri: 'https://legendmural.netlify.app/sport-lions-pride.html',
    storage,
...
test('does not persist unsafe or repository-prefixed registry image paths', async () => {
  ...
  for (const unsafeImage of [
    ...
    '/legend-stories-website/assets/beast.webp',
  ]) {
    ...
    const recovered = await recoverCartImage({
      ...
      page,
      storage,
      registryLoader: async () => [{ page, browserImage: unsafeImage }],
    });
